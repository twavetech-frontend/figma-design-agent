...
| `src/renderer/components/SettingsPanel.tsx` | 설정 UI |
...
- `src/main/settings-store.ts`에서 설정 읽기/쓰기 헬퍼 제공
...
- **변수 치환**: FAB(label/icon), Ribbon(text), Hero(banners[tag/title]), TabBar(activeTab)
...
### 4. Underline Tab Active/Inactive 높이 일치 + Individual Stroke
...
### 5. Tab Bar 아이템 — vertical FILL + 텍스트 CENTER 정렬
...
### 6. ⚠️ 모든 섹션/카드/리스트는 반드시 FILL 가로 사이징 (절대 HUG 금지)
...
### 7. ⚠️ Tab Bar와 FAB — 루트 프레임 하단에 배치 (콘텐츠 아래)
...
### 9. ⚠️ 카드 내 레이블/버튼 텍스트는 반드시 가시적으로
...
### 10. 섹션 간 간격 — 배경색 동일 + divider 없으면 gap 0
...
### 13. ⚠️ 스크린샷 QA — PRD 모든 섹션 보이는지 확인 필수
...
### 14. ⚠️ imin_home은 Footer Section 필수 (R31)
...
### 15. ⚠️ Horizontal carousel은 마지막 카드 ≥25% peek 강제 (R36)
...
### 16. ⚠️ Underline tab nav active 표시 = 2px brand bottom stroke (R35)
...
### 17. ⚠️ CTA/Button 프레임 — autoLayout에 paddingTop/Bottom 필수
...
> **`build` 완료 시 자동 실행** — `cmd_build`의 Step F가 post-fix 후
...
| [`docs/multi-agent-design.md`](docs/multi-agent-design.md) | 복잡한 화면(섹션 3+) 디자인 시 — 멀티에이전트 모드 |
